...
import { GameWiseCompletePrint } from 'utils/game/gameService'; // Adjust the import path as needed
...
const CompleteGamePrint: React.FC = () => {
  ...
  const fetchStartedDetails = async () => {
    try {
      if (id) {
        const result = await GameWiseCompletePrint(Number(id));
...
export default CompleteGamePrint;